...
class _DigitalAlbumListScreenState extends State<DigitalAlbumListScreen> {
  ...
  Future<void> fetchAlbums() async {
    ...
    final dio = Dio();
    final res = await dio.get('http://api.puzzlelog.me/api/albums', queryParameters: {'userId': userId});
  ...
  Future<void> handleDelete(String id) async {
    final dio = Dio();
    final res = await dio.delete('http://api.puzzlelog.me/api/albums/$id');

    if (res.statusCode == 200 && res.data['success']) {
      setState(() {
        albums.removeWhere((album) => album['_id'] == id);
      });
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('앨범이 삭제되었습니다.')));
    } else {
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: Text('나만의 디지털 앨범', style: Theme.of(context).textTheme.headlineSmall),
          ),
          Expanded(
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, idx) {
                      ...
                      return ListTile(
                        ...
                        onTap: () {
                          Navigator.pushNamed(context, '/albumDetail', arguments: {'albumId': album['_id']});
                        },
                      );
                    },
                  ),
