...
class _PostListScreenState extends State<PostListScreen> {
  ...
  Future<void> fetchPosts() async {
    final dio = Dio();
    try {
      final response = await dio.get("http://api.puzzlelog.me/posts");
      if (response.statusCode == 200) {
        ...
        for (var post in fetchedPosts) {
          final commentCountRes = await dio.get(
              "http://api.puzzlelog.me/posts/${post['id']}/comments/count");
  ...
  void toggleFilter(String selectedFilter) {
    setState(() {
      ...
      if (filter == 'mine') {
        filteredPosts = posts.where((post) => post['userId'] == userId).toList();
      } else {
        filteredPosts = posts;
      }
    });
        ...
        children: [
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text('커뮤니티', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                ElevatedButton(
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: filter == 'mine' ? Colors.brown : Colors.grey,
                ),
                      ...
                      return ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            Text('작성일: ${DateTime.parse(post['createdAt']).toLocal()}'),
                          ],
                        ),
                        onTap: () => Navigator.pushNamed(context, '/postDetail', arguments: post['id']),
                      );
                    },
                  )
