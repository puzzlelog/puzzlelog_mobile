...
class DiaryCanvasEditorState extends State<DiaryCanvasEditor> {
  ...
  Future<void> _fetchStickers() async {
    ...
    if (res.statusCode == 200 && res.data['success']) {
      ...
      setState(() {
        stickers =
            data
                .where((s) => s['type'] != 'AD' && s['type'] != 'background')
                .toList();
        backgroundImages =
            data.where((s) => s['type'] == 'background').toList();
      });
    }
  }
  ...
  void _toggleEraser() {
    setState(() {
      final isEraser =
          _controller.freeStyleSettings.mode == FreeStyleMode.erase;
    ...
    return Row(
      ...
      children: [
        IconButton(
          ...
          onPressed: () => setState(() => currentMode = 'pen'),
        ),
        IconButton(
        ...
        IconButton(
          ...
          onPressed: () => setState(() => _controller.clearDrawables()),
        ),
      ],
    );
  }
                    ...
                    (context) => AlertDialog(
                      content: SingleChildScrollView(
                        child: BlockPicker(
                          ...
                          onColorChanged:
                              (color) => Navigator.pop(context, color),
                        ),
                      ),
                    ),
              );
      ...
      builder:
          (_) => GridView.builder(
            ...
            itemBuilder: (_, idx) {
              ...
              return GestureDetector(
                onTap: () {
                  _setBackground(bg['mediaId']);
                  Navigator.pop(context);
                },
                child: Image.network(bg['mediaId']),
              );
            },
          ),
    );
