...
class _PieceBoxScreenState extends State<PieceBoxScreen> {
  ...
  Future<void> fetchPieces() async {
    ...
    final url = Uri.parse(
        'http://api.puzzlelog.me/pieces?userId=$userId&isDeleted=false&page=0&size=100');
  ...
  Future<void> handleDelete(String pieceId) async {
    ...
    final response = await http.delete(
        Uri.parse('http://api.puzzlelog.me/pieces/$pieceId'));
    ...
    if (response.statusCode == 200 && data['success']) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(data['message'] ?? '삭제 실패')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    final filteredPieces = filterType == "ALL"
        ? pieces
    ...
    final paginatedPieces = filteredPieces.sublist(
        start, end > filteredPieces.length ? filteredPieces.length : end);
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      Wrap(
                        spacing: 8,
                        children: ["ALL", "TEXT", "IMAGE", "VIDEO", "AUDIO"]
                            .map((type) => ChoiceChip(
